...
eps = 0.00001;
den = np.zeros((n_classes,1));
covinv = np.zeros((n_classes,IMAGE_PIXELS,IMAGE_PIXELS));

for i in range(n_classes):
	covinv[i] = np.linalg.pinv(cov[i]);
	den[i] = 1.0/ ( np.sqrt(np.linalg.det(cov[i])) + eps ) ;
den = np.log(den);
...
for i in range(len(x_test)):	
	...
	for j in range(n_classes):
		...
		sample = x_test[i];
		sample = sample - mu[j,:];
		temp = np.matmul( np.matmul(sample.T,covinv[j]) ,sample);
